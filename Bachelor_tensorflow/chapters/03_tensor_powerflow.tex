...
Zunächst wird der Algorithmus (\cref{sec:tpf_algorithmus}) eingeführt, welcher darauf folgend in zwei Formulierungen (Dense und Sparse) vertieft wird. 
...
\begin{equation}
	\tens{S} \in \C^{\ldots \times p \times r \times t \times b\phi} \xrightarrow{\text{reshape}} \dot{\mathbf{S}} \in \C^{b\phi \times \tau}
	\label{eq:reshape}
\end{equation}
...
Die Iteration erfolgt nach dem \emph{reshape} gemäß
