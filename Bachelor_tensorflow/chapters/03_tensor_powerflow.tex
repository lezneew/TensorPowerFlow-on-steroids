...
Der TPF basiert auf dem in \cref{sec:fpi} eingeführten FPI-Algorithmus nach Giraldo et al.\ \cite{Giraldo2022}. Ausgangspunkt ist das Netzmodell aus \eqref{eq:admittance_block} mit einem Einspeiseknoten, $b = |\Omega_d|$ Lastknoten und $\phi = |\Omega_\phi|$ Phasen. Die Iterationsvorschrift lautet \cite{Salazar2024}:
\begin{equation}
...
Dabei bezeichnen $\alpha_P$, $\alpha_I$ und $\alpha_Z$ die Koeffizienten des ZIP-Lastmodells (\cref{subsec:lastmodell}), $\vect{s} \in \C^{b\phi \times 1}$ den Vektor der komplexen Knotenleistungen. Die Matrizen $\vect{A} \in \C^{b\phi \times b\phi}$, $\vect{B} \in \C^{b\phi \times b\phi}$ und der Vektor $\vect{c} \in \C^{b\phi \times 1}$ sind für einen gegebenen Betriebspunkt \textit{konstant} innerhalb des iterativen Prozesses. Folglich können $\vect{F} \in \C^{b\phi \times b\phi}$ und $\vect{w} \in \C^{b\phi \times 1}$ einmal vorausberechnet und in jeder Iteration wiederverwendet werden.\\
...
Der Term $\vect{s}^* \had \vect{v}_{(n)}^{*\eleminv} = \vect{s}^* \oslash \vect{v}_{(n)}^*$ entspricht physikalisch dem konjugierten Stromvektor $\vect{i}^* = \vect{s}^*/\vect{v}^*$, wobei die Beziehung $s_k = v_k \cdot i_k^*$ aus \eqref{eq:power_current} genutzt wurde. Die Multiplikation $\ZB \cdot \vect{i}^*$ liefert den Spannungsabfall über die Netzimpedanzen, und $\vect{w}$ korrigiert für den Einfluss der festen Slack-Spannung $\vect{v}_s$.
%
...
Gleichung \eqref{eq:dense_matmul} stellt eine Multiplikation einer dichten Matrix $\ZB \in \C^{b\phi \times b\phi}$ mit einer Matrix $\in \C^{b\phi \times \tau}$ dar. Alle $\tau$ Lastflüsse werden durch \textit{eine einzige} Matrix-Matrix-Multiplikation gleichzeitig berechnet.
...
Es ist anzumerken, dass der Reshape von \eqref{eq:tensor_fpi} zu \eqref{eq:dense_matmul} die Konvergenz des FPI-Algorithmus nicht beeinflusst, da die Aktualisierung der Spannungswerte identisch zu \eqref{eq:tpf_iteration} bleibt \cite{Salazar2024}.
