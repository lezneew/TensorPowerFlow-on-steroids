...
In den vorherigen Kapiteln wurde gezeigt, dass der Tensor Power Flow durch die Konstantheit der Systemmatrix $\vect{F}$  einen erheblichen Geschwindigkeitsvorteil gegenüber konventionellen Verfahren bietet. Die zentrale Limitation liegt in der Annahme, dass der Leistungsvektor $\vect{s}$ über alle Iterationen vollständig bekannt und konstant ist (\cref{subsec:fpi_limitation,sec:tpf_limitation}). PV-Knoten verletzen diese Annahme, da ihre Blindleistung $Q_k$ unbekannt ist und iterativ bestimmt werden muss.
...
\begin{enumerate}
	...
	\item Berechne $\vect{s}^{(\ell)}$, daraus $\vect{A}^{(\ell)}$,
	...
	\emph{gekoppelt} über alle PV-Knoten
	(\S~\ref{subsec:methode_a_herleitung}).
...
Aus Sicht eines PV-Knotens~$k$ lässt sich das restliche Netz durch sein
Thévenin-Äquivalent beschreiben. Die Spannung am Knoten~$k$ ergibt sich als
\begin{equation}
...
$S_k = P_k + \jj Q_k = V_k\cdot I_k^*$. Daraus folgt für den Strom:
\begin{equation}
...
\begin{equation}
	...
	\;=\; Z^{(\ell)}_{B,kk}\cdot\frac{\jj\,\Delta Q_k}{(v^{(\ell)}_k)^*}.
...
%======================================================================
\paragraph{Berücksichtigung der Kopplung zwischen den PV-Knoten: }
