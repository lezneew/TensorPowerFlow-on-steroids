...
\end{equation}
Dabei bezeichnet $\mathbf{v}_s \in \mathbb{C}^{|\Omega_s|}$ die bekannten Slack-Spannungen und $\mathbf{v}_d \in \mathbb{C}^{b}$ die unbekannten Spannungen an den Lastknoten. Das negative Vorzeichen bei $\mathbf{i}_d$ folgt aus der Konvention, dass Einspeisungen positiv, Bezüge (Lasten) negativ gezählt werden.
...
\end{equation}
Die Beziehung zwischen komplexer Knotenleistung $\mathbf{s}_d$ und Knotenstrom $\mathbf{i}_d$ ist elementweise durch
\begin{equation}
...
\end{equation}
gegeben, wobei $\odot$ das Hadamard-Produkt, $\oslash$ die Hadamard-Division und $(\cdot)^{*}$ die komplexe Konjugation bezeichnet. Die Nichtlinearität des Lastflussproblems entsteht ausschließlich durch das Produkt $\mathbf{v}_d \odot \mathbf{i}_d^{*}$ in \eqref{eq:power_current}. Einsetzen in \eqref{eq:current_balance} liefert die Grundgleichung des Lastflusses:
\begin{equation}
	\label{eq:pf_fundamental}
	-\operatorname{diag}(\mathbf{v}_d^{*})^{-1}\mathbf{s}_d^{*} = \mathbf{Y}_{ds}\mathbf{v}_s + \mathbf{Y}_{dd}\mathbf{v}_d
\end{equation}
Diese Formulierung ist allgemein und gilt sowohl für einphasige und mehrphasige als auch für radiale und vermaschte Netze~\cite{Salazar2024}. Sie bildet den Ausgangspunkt für die in Kapitel~\ref{sec:tpf} vorgestellte Fixpunkt-Formulierung.
...
\begin{table}[h]
	\centering
	\caption{Knotentypen im Lastflussproblem}
	...
	\begin{tabular}{lccp{8.9cm}}
		...
		PQ    & $P,Q$        & $|V|,\theta$ & Haushalts- und Gewerbelasten, Industrieabnehmer, kleine PV-Anlagen im $\cos\varphi$-Betrieb, ungeregelte Windkraftanlagen, konventionelle Verbraucher \\
		PV    & $P,|V|$      & $Q,\theta$ & Synchrongeneratoren mit Erregerregelung (Wasser-, Gas-, Dampfkraftwerke), größere PV-Anlagen und Batteriewechselrichter mit Q(U)-Regelung nach VDE-AR-N 4110/4120, STATCOM-Anlagen \\
		\bottomrule
	\end{tabular}
\end{table}
\todo{literatur.}
...
\end{equation}
Dabei ist $\mathbf{s}_n$ die Nennleistung des Verbrauchers und $\boldsymbol{\alpha}_P, \boldsymbol{\alpha}_I, \boldsymbol{\alpha}_Z$ sind die knotenweisen ZIP-Koeffizienten mit
\begin{equation}
	\alpha_{P,k} + \alpha_{I,k} + \alpha_{Z,k} = 1 \quad \forall k \in \Omega_d
\end{equation}
Der Fall $\boldsymbol{\alpha}_P = \mathbf{1}$, $\boldsymbol{\alpha}_I = \boldsymbol{\alpha}_Z = \mathbf{0}$ entspricht dem \emph{konstanten Leistungsmodell}, welches in dieser Arbeit als Standardfall angenommen wird. Diese Annahme ist konsistent mit der Praxis in \texttt{pandapower} und ermöglicht die entscheidende Vereinfachung, dass die Systemmatrix der Fixpunkt-Iteration szenarienunabhängig konstant wird (siehe Abschnitt~\ref{sec:fpi}).
